...
// --- Helper Functions ---
// Cleans raw data row-by-row, coercing types and handling missing values.
const cleanData = (d) => {
    ...
    // Also check some key categorical fields to ensure data integrity
...
const App = () => {
    ...
    }, [dataUrl]); // Added dataUrl as a dependency
    ...
    const renderChart = () => {
        ...
        if (data.length === 0 && !loading) return <div className="flex items-center justify-center h-full"><p className="text-lg text-red-500">Failed to load data. Please check console and file path.</p></div>;
